...
import { DEV_ADMIN_ADDRESS, DEV_PLAYER1_ADDRESS, DEV_PLAYER2_ADDRESS, RUNTIME_SIMULATION_SOURCE, getActiveHorizonUrl, getActiveFriendbotUrl } from '@/utils/constants';
...
async function ensureTestnetAccountFunded(address: string): Promise<void> {
  ...
  const fundRes = await fetch(`${getActiveFriendbotUrl()}?addr=${address}`, { method: 'GET' });
  if (!fundRes.ok) {
