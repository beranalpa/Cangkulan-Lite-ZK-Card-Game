...
/**
 * Seed Stats Script — Pedersen Mode (Mode 4, BLS12-381)
 *
 * Identical to seed-stats.ts but uses Pedersen commitment + Schnorr/Sigma
 * proofs instead of NIZK hash-based proofs. Produces 224-byte proofs.
 *
 * Usage:
 *   bun run seed:pedersen              # Run once
 *   bun run seed:pedersen --loop       # Play 1 game every 5 minutes
 *   bun run seed:pedersen --loop 3     # Play 1 game every 3 minutes
 *   bun run seed:pedersen --wallets 8  # Use 8 wallets (default: 6)
 */
...
// ═══════════════════════════════════════════════════════════════════════════════
//  Crypto Helpers — Pedersen (BLS12-381) + Schnorr/Sigma
// ═══════════════════════════════════════════════════════════════════════════════
...
/**
 * Compute Pedersen commit hash: keccak256(C_bytes) → 32 bytes stored on-chain.
 */
...
/** Play commit hash (unchanged — card plays always use keccak) */
...
// ═══════════════════════════════════════════════════════════════════════════════
//  Wallet Management (shared with seed-stats.ts)
// ═══════════════════════════════════════════════════════════════════════════════
...
async function revealSeed(sessionId: number, player: Keypair, seedData: SeedData): Promise<void> {
  const proof = buildPedersenProof(
    seedData.seedHash,
    seedData.blinding,
    sessionId,
    player.publicKey(),
...
async function playFullGame(
  ...
  // 2. Commit seeds (Pedersen)
  console.log('  🌱 Committing seeds (Pedersen BLS12-381)...');
  const seed1 = prepareSeed(p1Addr);
  const seed2 = prepareSeed(p2Addr);

  await commitSeed(sessionId, p1, seed1.commitHash);
  console.log(`     P1 seed committed (Pedersen commit_hash)`);
  await commitSeed(sessionId, p2, seed2.commitHash);
  console.log(`     P2 seed committed (Pedersen commit_hash)`);

  // 3. Reveal seeds (Pedersen Schnorr/Sigma proof — 224 bytes)
  console.log('  🔓 Revealing seeds (Pedersen Schnorr/Sigma ZK proof — 224 bytes)...');
  await revealSeed(sessionId, p1, seed1);
  console.log(`     P1 seed revealed (Pedersen ✓)`);
  await revealSeed(sessionId, p2, seed2);
  console.log(`     P2 seed revealed (Pedersen ✓) → deck shuffled & dealt!`);
...
async function main() {
  ...
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('  🔐 Cangkulan Seed Stats — Pedersen Mode (BLS12-381)');
  console.log('═══════════════════════════════════════════════════════════════');
  console.log(`  Contract:   ${CONTRACT_ID.slice(0, 12)}...`);
  console.log(`  Proof Mode: Pedersen + Schnorr/Sigma (224 bytes)`);
